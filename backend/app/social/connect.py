...
def respond(user_id: int, connection_id: int, accept: bool) -> bool:
    with db.tx() as c:
        cur = c.execute("UPDATE connections SET status = ?, responded_at = ? WHERE id = ? AND to_user = ? AND status = 'pending'",
                        ("accepted" if accept else "declined", _now(), connection_id, user_id))
...
def open_reports() -> list[dict]:
    with db.tx() as c:
        rows = c.execute(
            "SELECT r.*, t.display_name AS target_name, t.status AS target_status, "
    ...
    return [{"id": r["id"], "target_id": r["target_id"], "target_name": r["target_name"], "target_status": r["target_status"], "reason": r["reason"],
             "detail": r["detail"], "message": r["message_body"], "open_against": r["open_against"], "created_at": r["created_at"]} for r in rows]
