...
| F-220 | **Search and autocomplete for trip ideas**: long idea lists now have a search box with suggestions. It matches names, descriptions, tags, interests, place types, cost labels and time-of-day hints, so searches like "beach", "indoor" or "rooftop" narrow both Ideas for your trip and the Add to day sheet | Built | React | `components/PlanSheets.jsx`, `pages/Trip.jsx` |
| F-221 | **Trip routes with per-city stay allocation**: the intake can collect several stops (cities/countries) for one trip. Prompt parsing turns named cities like "Paris, Rome and Athens" into an ordered route; the manual form lets travelers add/remove route stops. The planner splits nights across the stops, searches hotels and guides for each city segment, exposes `stay_segments` with dates/day ranges/options, combines ideas across the route, and lets the Stays page compare/select hotels per city | Built | React, FastAPI | `pages/Home.jsx`, `pages/Stays.jsx`, `app/schemas.py`, `app/live/llm.py`, `app/nodes.py` |
